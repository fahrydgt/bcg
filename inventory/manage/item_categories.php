...
{
//    display_error($_POST['parent_category']); die;
	...
	if (strlen($_POST['category_code']) == 0) 
	{
		$input_error = 1;
		display_error(_("The item category code cannot be empty."));
		set_focus('category_code');
	}
	...
	if ($input_error !=1)
	{
    	if ($selected_id != -1) 
    	{
		    update_item_category($selected_id, $_POST['parent_category'],$_POST['description'],$_POST['category_code'],
    	...
    	{
		    add_item_category($_POST['parent_category'], $_POST['description'],$_POST['category_code'],
				$_POST['tax_type_id'],	$_POST['sales_account'], 
				$_POST['cogs_account'], $_POST['inventory_account'], 
				$_POST['adjustment_account'], $_POST['wip_account'], 
...
if ($fixed_asset) {
	$th = array(_("Name"),_("Code"),_("parent Category"), _("Tax type"), _("Units"), _("Sales Act"),
		_("Asset Account"), _("Deprecation Cost Account"),
		_("Depreciation/Disposal Account"), "", "");
} else {
	$th = array(_("Name"),_("Code"),_("parent Category"), _("Tax type"), _("Units"), _("Type"), _("Sales Act"),
...
$prnt_cat_list = get_parent_cats();

while($cat1 = db_fetch($prnt_cat_list))
        $parent_cat_list[$cat1['parent_cat_code']] = $cat1['parent_cat_name'];
...
while ($myrow = db_fetch($result)) 
{
	...
	label_cell($myrow["category_code"]);
	label_cell($parent_cat_list[$myrow["parent_id"]]);
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$_POST['parent_category']  = $myrow["parent_id"];
		$_POST['description']  = $myrow["description"];
		$_POST['category_code']  = $myrow["category_code"];
...
text_row(_("Category Code:"), 'category_code', null, 30, 30);  
stock_parent_cat_list_row(_("Parent Category:"), 'parent_category', null);
